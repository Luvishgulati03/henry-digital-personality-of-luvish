 ...
 *
 * 1. NOT PROOF OF AUTHORITY. This module returns a transcript; it never acts on it and never
 *    decides whether the words may act. The bridge (src/telegram/bridge.ts) runs the transcript
 *    as a VOICE TURN, which is the same read-only, HENRY_VOICE_TURN-flagged rail a spoken
 *    dashboard turn gets (src/voice/policy.ts): a transcript can ask, but cannot approve,
 *    execute, or send, no matter what it says.
 ...
 *
 * Doctrine rule 7: this file imports no Henry module except the wire types it consumes.
...
/** The transcription contract, satisfied structurally by LocalVoiceService (src/voice/index.ts). */
...
export interface VoiceIntakeLimits {
  ...
  language: string;
  /** Optional Henry vocabulary hint passed straight through to the adapter's --prompt. */
  prompt?: string;
}
 ...
 * temp directory that is always removed. Henry never installs or downloads ffmpeg: the
 * operator names an existing executable (HENRY_FFMPEG_PATH), or voice stays off.
 */
export function ffmpegAudioConverter(options: FfmpegConverterOptions): AudioConverter {
  ...
  const run = async (input: Buffer, args: (inPath: string, outPath: string) => string[], outName: string): Promise<Buffer> => {
    const dir = await mkdtemp(path.join(options.tempRoot ?? os.tmpdir(), "henry-tg-voice-"));
    try {
 ...
 *
 * Henry answers in text first, always. A voice reply is an extra, and it is attempted only
