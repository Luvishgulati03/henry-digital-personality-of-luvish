...
/**
 * `henry tunnel setup <hostname>`: a one-time, idempotent setup flow for putting Henry's PUBLIC
 * face on the owner's own Cloudflare domain (`https://henry.<your-domain>`). Ported from Kelly.
 * It never passes `--overwrite-dns`: a hostname already routed elsewhere stops with a message.
...
/** Lowercase DNS hostname, no scheme, no path, at least one dot (e.g. `henry.example.com`). */
export function validateHostname(raw: string): string {
  ...
  if (!pattern.test(hostname)) {
    throw new Error(`"${hostname}" does not look like a DNS hostname, e.g. henry.example.com`);
  }
  return hostname;
}
...
/** HENRY_CLOUDFLARED_PATH, then PATH, then the two common Homebrew install locations. */
export async function findCloudflared(deps: CloudflareSetupDeps): Promise<string | undefined> {
  const custom = deps.env.HENRY_CLOUDFLARED_PATH?.trim();
  if (custom) {
  ...
  if (result.exitCode !== 0) {
    throw new Error(
      `cloudflared tunnel login did not finish successfully (exit code ${String(result.exitCode)}). ` +
        "Run `henry tunnel setup <hostname>` again once you have picked a domain in the browser.",
    );
  }
}
  ...
  if (conflict) {
    ...
    throw new Error(
      `${hostname} is already routed to something else in Cloudflare DNS. Delete that DNS record in the ` +
        "Cloudflare dashboard, or choose a different hostname, then run `henry tunnel setup` again.",
    );
  }
...
/** Runs the full idempotent setup flow described in docs/modules/public-mode.md. */
export async function runCloudflareTunnelSetup(rawHostname: string, options: CloudflareTunnelSetupOptions, deps: CloudflareSetupDeps): Promise<void> {
  ...
  const name = options.name?.trim() || "henry";
  ...
  // HENRY_TUNNEL is deliberately NOT written: the public link only ever comes up through an
  // explicit `henry start --public`, never because a plain `henry dashboard` read .env.
  await writeEnvUpdates(deps, {
    HENRY_CLOUDFLARE_TUNNEL: name,
    HENRY_PUBLIC_HOST: hostname,
  });

  deps.log("");
  deps.log("Henry's public face is configured for your own domain. Next:");
  deps.log("  1. Publish the public knowledge pack (henry public pack ...).");
  deps.log("  2. Fill in the \"## Public mode (recruiters and visitors)\" section of soul.md.");
  deps.log("  3. henry start --public");
  deps.log(`Link: https://${hostname}`);
}

/** `henry tunnel setup --status`: never prints cert.pem contents, only whether it exists. */
export async function runCloudflareTunnelStatus(deps: CloudflareSetupDeps): Promise<CloudflareStatusReport> {
  ...
  const tunnelName = parsed.HENRY_CLOUDFLARE_TUNNEL || undefined;
  const publicHost = parsed.HENRY_PUBLIC_HOST || undefined;
  ...
  deps.log(`Tunnel: ${tunnelName ? `${tunnelName} (${tunnelExists ? "exists" : "not found"})` : "(HENRY_CLOUDFLARE_TUNNEL not set)"}`);
  deps.log(`HENRY_PUBLIC_HOST: ${publicHost ?? "(not set)"}`);
