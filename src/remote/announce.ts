...
export const TUNNEL_STILL_CONNECTING_MESSAGE =
  "Remote access is still connecting; the link will work once Cloudflare registers the tunnel (check with henry tunnel status).";
...
export function connectedLines(status: TunnelStatus, reconnected: boolean): TunnelAnnounceLine[] {
  ...
  if (status.public && !reconnected) {
    lines.push({ level: "info", text: "Public link: anyone with the URL can chat with Henry's public face. Only the public chat and talk pages are reachable; the dashboard stays local." });
  }
  return lines;
}
 ...
 * remote.stopped (a deliberate shutdown) is not a drop and prints nothing.
