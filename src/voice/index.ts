 ...
 *     tts: { engine: "kokoro", url: process.env.HENRY_KOKORO_URL, token: process.env.HENRY_KOKORO_TOKEN },
 *   });
 *   const { text, language } = await voice.transcribe(wavBytes, { language: "hi" });
 *   const wavBytes = await voice.synthesize(text, { language: "hi" });
 *
 * Use voiceConfigFromEnv() to map HENRY_WHISPER_CPP_PATH, HENRY_WHISPER_MODEL_PATH,
 * HENRY_TTS_ENGINE, HENRY_TTS_EXECUTABLE, HENRY_TTS_MODEL_PATH, HENRY_KOKORO_URL and
 * HENRY_KOKORO_TOKEN. Text is passed through verbatim: no translation, script conversion,
 * or number/quantity normalization is performed. Inputs and outputs are WAV.
 */
...
export class LocalVoiceService {
  ...
  async transcribe(wav: Uint8Array, options: TranscriptionOptions = {}): Promise<TranscriptionResult> {
    ...
    const dir = await mkdtemp(path.join(this.config.tempRoot ?? os.tmpdir(), "henry-stt-"));
    try {
  ...
  async synthesize(text: string, options: SynthesisOptions = {}): Promise<Buffer> {
    ...
    const dir = await mkdtemp(path.join(this.config.tempRoot ?? os.tmpdir(), "henry-tts-"));
    try {
...
/** Read supported Henry environment settings without making any engine implicit. */
export function voiceConfigFromEnv(env: NodeJS.ProcessEnv = process.env): VoiceConfig {
  const stt: SttConfig = {
    whisperCppPath: env.HENRY_WHISPER_CPP_PATH || undefined,
    whisperModelPath: env.HENRY_WHISPER_MODEL_PATH || undefined,
  };
  const requestedEngine = env.HENRY_TTS_ENGINE || (env.HENRY_KOKORO_URL ? "kokoro" : undefined);
  const tts: TtsConfig = {
    ...
    executablePath: env.HENRY_TTS_EXECUTABLE || undefined,
    modelPath: env.HENRY_TTS_MODEL_PATH || undefined,
    url: env.HENRY_KOKORO_URL || undefined,
    token: env.HENRY_KOKORO_TOKEN || undefined,
  };
  return {
