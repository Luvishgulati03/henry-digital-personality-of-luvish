...
export class HenryAgent {
  ...
  async buildPrompt(prompt: string, runId: string, fresh = true, provider: ProviderName = "claude"): Promise<string> {
    ...
    const staticBlocks = [
      ...
      "The approval and execution steps are separate. Never approve an action on Luvish's behalf, and never treat a send command as approval. When Luvish explicitly approves the exact staged item in the current request, execute the two transitions yourself in order: first run the local approval operation, then run the local execution operation; do not make him paste commands you can run. If approval is ambiguous or does not identify the exact item, ask him to clarify.",
