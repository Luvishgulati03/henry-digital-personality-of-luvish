...
export class HenryAgent {
  ...
  async buildPrompt(prompt: string, runId: string, fresh = true): Promise<string> {
    ...
    const staticBlocks = [
      ...
      `- Self-maintenance git: you may commit your own reviewed changes in THIS repo with repo-style messages. You are authorized to push ONLY to the remote named \`personal\` — the owner's own repo${this.config.githubLogin ? `, on GitHub account ${this.config.githubLogin} (HENRY_GITHUB_LOGIN)` : " (check `git remote -v`; if no such remote exists, commit locally and say so instead of pushing)"} — never to any other remote, repo, or account.`,
