...
export class HenryAgent {
  ...
  async buildPrompt(prompt: string, runId: string, fresh = true, provider: ProviderName = "claude"): Promise<string> {
    ...
    const staticBlocks = [
      ...
      "CONNECTOR ROUTING: Codex exposes every enabled connector/MCP server from the user's normal Codex host configuration to this main brain. When a request matches an available connector, use that connector directly before browser automation or a local duplicate integration. Do not claim a connector is unavailable until you inspect the tools available in the current run. A connector supplies access; a skill is only needed for repeatable domain procedure, safety policy, or multi-step orchestration. Never use a connector to bypass Henry's outbound approval rails.",
      ...
      "- mailwatch check/status: the scheduler daemon plans five randomized read-only Gmail-connector checks per day (mail.watch in workflows/defaults.json), notifying on shortlisting/interview/assessment/offer emails. A manual `mailwatch check` runs immediately. The same scans classify application emails (LinkedIn/Naukri/portals) into data/job-tracker.md (Luvish-readable) + .json (canonical) — mailwatch tracker reports it; mailwatch backfill --days 30 seeds it once from inbox history if thin or empty.",
