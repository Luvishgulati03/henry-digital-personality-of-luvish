...
(function (global) {
  ...
  var CSS = [
    ...
    /* pan-y (not none): a single-finger vertical swipe on a phone still scrolls the
       page past the graph instead of getting trapped inside the canvas. Horizontal
       drag still reaches JS for yaw; the on-screen pad covers pitch on touch. */
    '.hmc-canvas{display:block;width:100%;height:100%;touch-action:pan-y;cursor:grab;outline:none}',
    ...
    '.hmc-sub input[type=range]{width:100%;accent-color:var(--hmc-cyan);height:44px;margin:0}',
    ...
    '.hmc-btn{min-height:44px;display:inline-flex;align-items:center;gap:7px;padding:0 13px;border-radius:11px;',
    ...
    '.hmc-btn.icon{padding:0;width:44px;justify-content:center;font-size:13px}',
    '.hmc-select{min-height:44px;padding:0 11px;border-radius:11px;border:1px solid var(--hmc-line);',
    'background:rgba(9,20,38,.72);color:var(--hmc-text);font:16px ui-monospace,SFMono-Regular,Menlo,monospace}',
    '.hmc-select:focus-visible{outline:2px solid var(--hmc-cyan);outline-offset:2px}',
    '.hmc-pad{display:inline-grid;grid-template-columns:repeat(3,44px);grid-auto-rows:44px;gap:3px}',
    '.hmc-pad .hmc-btn{min-height:44px;width:44px;padding:0;justify-content:center;border-radius:7px;font-size:11px}',
