...
/**
 * THE PUBLIC SANDBOX: how a turn answering an anonymous visitor (src/public/) is spawned.
 *
 * A public turn must not be able to read ANY file, run anything, reach a connector or MCP server,
 * browse, or keep a session, whatever the visitor types ("cat ~/.env", "read soul.md", "what is in
 * your memory"). The persona rules and the published knowledge pack are inlined into the prompt by
 * the server; the model needs nothing else. Each CLI is locked down with its own documented flags,
 * and both run in an empty scratch directory outside the repository (src/public/turn.ts).
 *
 * Claude (verified live with Claude Code 2.1.x: the init event reports `tools: []`,
 * `mcp_servers: []`, no skills, no plugins):
 *   --tools ""                     no built-in tools at all (no Read/Grep/Glob/Bash/Web*)
 *   --safe-mode                    no CLAUDE.md, skills, plugins, hooks, MCP servers, custom agents
 *   --strict-mcp-config + empty --mcp-config   no MCP servers from any other configuration
 *   --setting-sources ""           no user/project/local settings files (no hooks, no permissions)
 *   --permission-mode dontAsk      any tool that still appeared would be denied (empty allowlist)
 *   --disallowedTools <list>       the built-in file/shell/web tools denied by name as well
 *   --disable-slash-commands, --no-session-persistence
 *   --system-prompt                replaces Claude Code's default agent prompt (which carries the
 *                                  working directory and platform) with Henry's public rules
 *
 * Codex (verified live with codex-cli 0.153: the model reports no shell or file-read tool, and a
 * write attempt is rejected by the sandbox):
 *   --disable shell_tool / unified_exec   no shell, so no way to read a file
 *   --disable apps, plugins, browser_use, computer_use, in_app_browser, view_image, multi_agent,
 *     image_generation, hooks, memories, goals, tool_suggest, skill_mcp_dependency_install
 *   --ignore-user-config           ~/.codex/config.toml is not loaded (no MCP servers, no profile)
 ...
 *
 * Both runs get a MINIMAL environment (publicEnvironment): no GitHub tokens, no HENRY_* keys, just
 * what the CLI needs to find its own subscription login, plus HENRY_PUBLIC_TURN=1 so every
 * approval/send path refuses even if a future CLI release handed the model a shell.
 *
...
export const CODEX_PUBLIC_DISABLED_FEATURES = [
  "shell_tool", "unified_exec", "apps", "plugins", "multi_agent", "browser_use", "computer_use",
  "in_app_browser", "view_image", "image_generation", "hooks", "memories", "goals", "tool_suggest",
...
/** Codex argv for a public turn. Codex has no separate system channel, so rules ride in the prompt. */
