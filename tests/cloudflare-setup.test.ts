...
test("validateHostname: accepts a plain lowercase DNS hostname", () => {
  assert.equal(validateHostname("henry-demo.example.com"), "henry-demo.example.com");
...
test("validateHostname: rejects a scheme, a path, uppercase, or a bare label", () => {
  assert.throws(() => validateHostname("https://henry-demo.example.com"), /scheme/);
  assert.throws(() => validateHostname("henry-demo.example.com/path"), /path/);
  assert.throws(() => validateHostname("Shop-Demo.example.com"), /lowercase/);
  assert.throws(() => validateHostname("henrytest"), /does not look like a DNS hostname/);
  assert.throws(() => validateHostname(""), /Usage: henry tunnel setup/);
});
...
test("findCloudflared: missing cloudflared surfaces a clear brew-install message", async () => {
  ...
  await assert.rejects(
    runCloudflareTunnelSetup("henry-demo.example.com", {}, deps),
    /brew install cloudflared/,
  );
});

test("findCloudflared: HENRY_CLOUDFLARED_PATH wins, then PATH, then the Homebrew fallbacks", async () => {
  const { deps: viaEnv } = fakeDeps({
    env: { HENRY_CLOUDFLARED_PATH: "/custom/cloudflared" },
...
test("setup: login is skipped when cert.pem already exists", async () => {
  ...
  const { deps, calls } = fakeDeps({
    ...
    run: async (_cmd, args) => {
      ...
      if (args[0] === "tunnel" && args[1] === "create") return { stdout: "Created tunnel henry-demo with id aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee\n", stderr: "", exitCode: 0 };
      if (args[0] === "tunnel" && args[1] === "route") return { stdout: "", stderr: "", exitCode: 0 };
      return { stdout: "", stderr: "", exitCode: 0 };
    },
  });
  await runCloudflareTunnelSetup("henry-demo.example.com", {}, deps);
  ...
  const { deps, calls } = fakeDeps({
    ...
    run: async (_cmd, args) => {
      if (args[0] === "tunnel" && args[1] === "list") {
        return { stdout: JSON.stringify([{ id: "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee", name: "henry-demo" }]), stderr: "", exitCode: 0 };
      }
      if (args[0] === "tunnel" && args[1] === "route") return { stdout: "", stderr: "", exitCode: 0 };
      return { stdout: "", stderr: "", exitCode: 0 };
    },
...
test("setup: a DNS route already pointing at this tunnel is treated as fine", async () => {
  ...
  const { deps } = fakeDeps({
    ...
    run: async (_cmd, args) => {
      if (args[0] === "tunnel" && args[1] === "list") {
        return { stdout: JSON.stringify([{ id: tunnelId, name: "henry" }]), stderr: "", exitCode: 0 };
      }
      if (args[0] === "tunnel" && args[1] === "route") {
...
test("setup: writes .env keeping other lines, replacing existing keys, with a .env.bak and mode 0600; never turns the tunnel on", async () => {
  const certPath = path.join("/home/fake", ".cloudflared", "cert.pem");
  ...
  files.set(envPath, "HENRY_PORT=7337\nHENRY_PUBLIC_HOST=old.example.com\nHENRY_TUNNEL=off\n");
  const chmodCalls: Array<{ path: string; mode: number }> = [];
  deps.chmod = async (p, mode) => { chmodCalls.push({ path: p, mode }); };

  await runCloudflareTunnelSetup("henry-demo.example.com", { name: "henry-demo" }, deps);

  const updated = files.get(envPath)!;
  assert.match(updated, /HENRY_PORT=7337/);
  // The public link only comes up through `henry start --public`: setup never writes HENRY_TUNNEL=cloudflare.
  assert.match(updated, /HENRY_TUNNEL=off/);
  assert.doesNotMatch(updated, /HENRY_TUNNEL=cloudflare/);
  assert.match(updated, /HENRY_CLOUDFLARE_TUNNEL=henry-demo/);
  assert.match(updated, /HENRY_PUBLIC_HOST=henry-demo\.example\.com/);
  assert.doesNotMatch(updated, /old\.example\.com/);
  assert.equal(files.get(backupPath), "HENRY_PORT=7337\nHENRY_PUBLIC_HOST=old.example.com\nHENRY_TUNNEL=off\n");
...
test("status: never prints cert.pem contents, only presence", async () => {
  ...
  const { deps, logs } = fakeDeps({
    ...
    readFile: async (p) => {
      ...
      if (p === envPath) return "HENRY_CLOUDFLARE_TUNNEL=henry-demo\nHENRY_PUBLIC_HOST=henry-demo.example.com\n";
      throw new Error(`ENOENT: ${p}`);
    },
    run: async (_cmd, args) => {
      if (args[0] === "tunnel" && args[1] === "list") {
        return { stdout: JSON.stringify([{ id: "abc", name: "henry-demo" }]), stderr: "", exitCode: 0 };
      }
      return { stdout: "", stderr: "", exitCode: 0 };
    },
  ...
  assert.equal(report.publicHost, "henry-demo.example.com");
...
test("setup + status: end-to-end against a temp HOME and a temp repo root", async () => {
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "henry-cf-home-"));
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "henry-cf-repo-"));
  try {
    const { deps } = fakeDeps({
      env: { PATH: "/usr/bin" },
      homeDir,
    ...
    assert.doesNotMatch(content, /HENRY_TUNNEL=/);
    assert.match(content, /HENRY_CLOUDFLARE_TUNNEL=henry/);
    assert.match(content, /HENRY_PUBLIC_HOST=henry-demo\.example\.com/);
    const stat = await fs.stat(envPath);
    assert.equal(stat.mode & 0o777, 0o600);

    const status = await runCloudflareTunnelStatus(deps);
    assert.equal(status.publicHost, "henry-demo.example.com");
    assert.equal(status.tunnelName, "henry");
  } finally {
...
test("setup: never passes --overwrite-dns to cloudflared", async () => {
  const certPath = path.join("/home/fake", ".cloudflared", "cert.pem");
  ...
  const { deps } = fakeDeps({
    ...
    run: async (_cmd, args) => {
      calls.push({ args });
      if (args[0] === "tunnel" && args[1] === "list") return { stdout: "[]", stderr: "", exitCode: 0 };
      if (args[0] === "tunnel" && args[1] === "create") return { stdout: "Created tunnel henry with id aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee\n", stderr: "", exitCode: 0 };
      return { stdout: "", stderr: "", exitCode: 0 };
    },
  });
  await runCloudflareTunnelSetup("henry.example.com", {}, deps);
  const route = calls.find((call) => call.args[0] === "tunnel" && call.args[1] === "route");
  assert.deepEqual(route?.args, ["tunnel", "route", "dns", "henry", "henry.example.com"]);
  assert.ok(calls.every((call) => !call.args.some((arg) => /overwrite/i.test(arg))));
});
