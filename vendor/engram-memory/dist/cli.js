...
function embeddingFromFlags(flags) {
    if (!flags.provider && !flags.dim)
        return undefined;
    const provider = flags.provider || "hashing";
    const dim = flags.dim ? Number(flags.dim) : undefined;
    if (provider === "openai") {
        return {
            provider: "openai",
            apiKey: flags["openai-key"] || process.env.OPENAI_API_KEY,
...
function llmFromFlags(flags) {
    const llm = flags.llm;
    if (!llm)
        return undefined;
    if (llm === "none")
        return { provider: "none" };
    const model = flags["llm-model"] || undefined;
    const useTmux = Boolean(flags.tmux);
    if (llm === "claude" || llm === "claude-cli")
        return { provider: "claude-cli", model, useTmux };
    if (llm === "codex" || llm === "codex-cli")
        return { provider: "codex-cli", model, useTmux };
    return undefined;
}
async function main() {
    ...
    const dbPath = flags.db || config.dbPath || process.env.ENGRAM_DB || "engram.db";
    ...
    try {
        switch (cmd) {
            case "index": {
                ...
                if (flags["llm-edges"]) {
                    if (!engram.llm) {
                        ...
                        process.stdout.write(`LLM edges: caused ${le.caused}, supersedes ${le.supersedes}, lesson_from ${le.lesson_from} ` +
            ...
            case "recall": {
                ...
                if (!query)
                    throw new Error('recall requires a query, e.g. recall "how do I X"');
                ...
                results.forEach((r, i) => {
                    ...
                    process.stdout.write(`${i + 1}. [score ${r.score.toFixed(4)}] ${snippet}\n` +
            ...
            case "add": {
                ...
                if (!text)
                    throw new Error("add requires text");
            ...
            case "dream": {
                ...
                if (p)
                    process.stdout.write(`Promoted ${p.promoted} short-term -> long-term (${p.eligible} eligible of ${p.scanned}).\n`);
                if (c)
                    process.stdout.write(`Consolidated: scored ${c.scored}, kept ${c.kept}, archived ${c.archived} (${c.protectedCount} protected).\n`);
                break;
            }
            case "promote": {
                ...
                const res = engram.promote({
                    ...
                    toTier: flags["to-tier"] || undefined,
                    dryRun: dry,
                });
                if (flags.json) {
                    process.stdout.write(`${JSON.stringify(res)}\n`);
                ...
                process.stdout.write(`Promotion: scanned ${res.scanned}, ${res.eligible} eligible, ` +
            ...
            case "eval": {
                ...
                if (!file)
                    throw new Error("eval requires a <file.json> labelled set");
                ...
                if (flags.tune) {
                    const t = await tuneWeights(engram, set, { semantic: [0.5, 1, 2], lexical: [0.5, 1, 2], importance: [0, 0.5, 1] }, { k, recall: { associative } });
                    process.stdout.write(`Tuned recall@${k}: ${t.baseline.toFixed(3)} → ${t.bestScore.toFixed(3)} ` +
            ...
            case "dashboard": {
                ...
                await new Promise(() => { }); // serve until interrupted (skips the finally close)
                break;
            }
            default:
