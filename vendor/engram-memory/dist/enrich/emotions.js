...
for (const fam of EMOTION_FAMILIES) {
    for (const m of fam.members) {
        if (!META.has(m))
            META.set(m, { family: fam.key, valence: fam.valence, hue: fam.hue, color: fam.color });
    }
}
/** Every emotion in the palette, flat and de-duplicated (lowercase). */
export const EMOTIONS = [...META.keys()];
/**
...
export function emotionPalette() {
    ...
    for (const fam of EMOTION_FAMILIES) {
        for (const m of fam.members)
            if (!p[m])
                p[m] = { color: fam.color, hue: fam.hue, valence: fam.valence, family: fam.key };
    }
    return p;
}
/** One swatch per family, valence-ordered, for an emotion legend (excludes plain "neutral"). */
export function emotionFamilyLegend() {
    const order = { positive: 0, ambivalent: 1, neutral: 2, negative: 3 };
...
export function emotionInfo(emotion) {
    if (!emotion)
        return undefined;
...
export function isKnownEmotion(emotion) {
 ...
 */
export function affectFromMetadata(metadata) {
    if (!metadata)
        return { intensity: 0, valence: "neutral" };
 ...
 */
export function emotionPalettePrompt() {
