...
export function walk(dir, extensions = DEFAULT_EXTENSIONS) {
    const out = [];
    const stack = [dir];
    while (stack.length) {
...
export function chunkContent(content, strategy) {
 ...
 */
export function canonicalTier(raw) {
    if (!raw)
        return undefined;
    const t = raw.trim().toLowerCase();
    if (t === "")
        return undefined;
    if (CANONICAL_TIERS.has(t))
        return t;
    // Hand-curated, durable knowledge (common agent memory taxonomies + synonyms).
    if (["feedback", "project", "reference", "user", "fact", "lesson", "rule", "note", "insight"].includes(t))
        return "semantic";
    // Time-stamped conversational / log entries.
    if (["conversation", "daily", "exchange", "session", "episode", "log", "message"].includes(t))
        return "episodic";
    // Step-by-step operational knowledge.
    if (["runbook", "howto", "procedure", "process", "checklist", "playbook"].includes(t))
        return "procedural";
 ...
 */
function defaultImportance(tier) {
    switch (tier) {
        case "semantic":
        case "procedural":
            return 0.7;
...
export function ingestFile(absPath, rootDir, opts = {}) {
    const raw = readFileSync(absPath, "utf8");
    const { data, body } = parseFrontmatter(raw);
    const hasFrontmatter = Object.keys(data).length > 0;
    const requested = opts.chunk ?? "auto";
    const strategy = requested === "auto" ? (hasFrontmatter ? "file" : "paragraph") : requested;
    ...
    const importance = typeof data.importance === "number" ? data.importance : defaultImportance(tier);
    const createdAt = parseDate(data.date) ?? parseDate(data.created_at) ?? statSync(absPath).mtimeMs;
...
export function ingestDirectory(dir, opts = {}) {
    const files = walk(dir, opts.extensions);
    const out = [];
    for (const f of files)
        out.push(...ingestFile(f, dir, opts));
    return out;
}
