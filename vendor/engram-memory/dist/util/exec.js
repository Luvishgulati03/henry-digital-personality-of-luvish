    ...
    return new Promise((resolve, reject) => {
        ...
        child.on("close", (code) => {
            ...
            if (code === 0)
                ...
                reject(new Error(`${cmd} exited ${code}: ${(err || out).slice(0, 500)}`));
        });
 ...
 */
export async function runViaTmux(bin, args, opts = {}) {
    ...
    cleanup();
    if (code !== "0")
        throw new Error(`tmux command exited ${code}: ${(err || out).slice(0, 500)}`);
    return out;
}
