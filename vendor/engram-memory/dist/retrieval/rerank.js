...
export function parseOrder(resp, n) {
    ...
    try {
        ...
        if (!Array.isArray(arr))
            return [];
        return arr.filter((x) => Number.isInteger(x) && x >= 1 && x <= n);
    }
    catch {
        return [];
    }
 ...
 */
export async function llmRerank(llm, query, candidates, k) {
    ...
    const list = candidates
        ...
        .join("\n");
    const prompt = `You are selecting the memories most relevant to a query.\n\n` +
        `Query: "${query}"\n\nMemories:\n${list}\n\n` +
