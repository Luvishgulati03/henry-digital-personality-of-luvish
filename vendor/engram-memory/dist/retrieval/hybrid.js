 ...
 */
export async function recall(store, provider, query, opts, baseWeights) {
    const w = { ...baseWeights, ...(opts.weights ?? {}) };
    const k = opts.k ?? 8;
    ...
    if (!opts.includeSuperseded)
        records = records.filter((r) => r.invalidAt == null);
    if (opts.tier)
        records = records.filter((r) => r.tier === opts.tier);
    const now = Date.now();
    const results = records.map((r) => {
        ...
        const why = [];
        if (e.semRank)
            why.push(`semantic #${e.semRank} (${(e.semScore ?? 0).toFixed(2)})`);
        if (e.lexRank)
            why.push(`lexical #${e.lexRank}`);
        why.push(`importance ${r.importance.toFixed(2)}`);
        if (freqTerm)
            why.push(`used ${r.useCount}×`);
        if (affect.intensity > 0)
            why.push(`${affect.emotion ?? "affect"} ${affect.intensity.toFixed(2)}`);
        return {
            id: r.id,
            content: r.content,
            source: r.source,
