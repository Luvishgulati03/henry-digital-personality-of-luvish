...
## Step −1 — Load the runbook, if one exists (do this first, every time)

Check for `memory/runbooks/pr-review.md` (and any repo-specific runbook named after the
target repo, e.g. `memory/runbooks/<repo>-review.md`). If `memory/runbooks/` exists and
contains a relevant file, read it before anything else — it defines the canonical
orchestration and step list for this repo: which channel to ack in, where to post
comments, how to format the verdict, any repo-specific non-negotiables.

`memory/runbooks/` is optional — most repos won't have one yet. If it's missing, fall
back to the methodology below as-is. If a runbook exists and conflicts with this agent
file, the runbook wins (it's the user-maintained source of truth; this file documents
the agent's default behaviour shape).
