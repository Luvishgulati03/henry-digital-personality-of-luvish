...
1. **Load rules.**
   - Read the repo's own CLAUDE.md / AGENTS.md. Non-negotiable.
   - Read the feature doc for the area you're working on, if one exists. If none exists, flag it.
...
2. Write it to a notes file (e.g. `learnings.md` at the repo root, if the repo keeps one).
3. Move on to the next task. Don't silently produce bad output.

## When multi-agent dispatch infrastructure exists

This agent assumes a single-turn worker. If the host repo has a dispatch/orchestration
layer (a lead/orchestrator agent, a task queue, a Slack-style thread), follow that
repo's own dispatch conventions for escalating blockers or handing off to a reviewer —
don't invent a protocol here.

## Done means

- The requested change or investigation is complete.
- Relevant verification ran, or the blocker is named.
- Any required agent dispatch happened, per the host repo's own conventions.
