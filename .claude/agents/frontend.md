...
1. **Load context.** Read the repo's own CLAUDE.md / AGENTS.md, check existing components before creating new ones, read the feature doc if one exists.
...
4. **Verify.**
   - Responsive: mobile, tablet, desktop (where applicable to the platform).
...
Mobile screens replace "responsive: mobile/tablet/desktop" with device/orientation
checks, and "keyboard navigation" with focus order for accessibility tooling. Check the
repo's own design-token/theming setup (see its `.claude/rules/` or equivalent) before
