...
For each repo with a usable local git checkout:
...
1. Establish the protected primary checkout and default branch.
   ...
   - Prefer `origin/main`, then `origin/master`, then the local branch with the same name.
   - Never construct refs like `origin/origin/main`.
2. Enumerate worktrees with `git worktree list --porcelain` from the primary checkout.
   ...
   - Skip locked worktrees, worktrees whose path no longer exists after pruning, and
     worktrees currently used by an active process.
3. Only prune a worktree when its checked-out HEAD is already contained in the default branch.
   ...
   - Do not delete unmerged branches, detached reviews, or unknown HEADs just because
     their working tree is clean.
4. Inspect working-tree changes before removal.
   - A clean merged worktree can be removed.
   - A merged worktree with only ignorable local artifacts can be removed. Ignorable
     artifacts are `.DS_Store`, `.claude/**`, `.codex/**`, and empty directories left
     behind by those artifacts.
   - If the only meaningful changed file is a `learnings.md`-style notes file, migrate
     any durable, non-duplicative knowledge into the equivalent file in the primary
     checkout (create it if missing), then remove the worktree if the remaining changes
     are only ignorable artifacts.
   - Do not copy secrets, raw logs, temporary command output, credentials, personal
     data, or transient chatter from a worktree's notes file.
   - If there are any other modified, deleted, staged, untracked, or ignored files that
     are not explicitly allowed above, skip the worktree and report the paths.
5. Remove only safe worktrees.
   - Prefer `git worktree remove <path>`.
   - Use `--force` only when the worktree is merged and all remaining changes are
     limited to the allowed ignorable artifacts or already-migrated notes.
...
6. Treat learnings migration as a repository change, not as an invisible side effect.
   - If the primary checkout's notes file changed, leave the file modified in the
     primary checkout and call that out in the final summary with the exact path.
   - Do not commit, push, or open a PR from this workflow. The operator should review
     migrated knowledge first.

Return a compact summary with:

- repos inspected
- worktrees removed, grouped by repo
- learnings migrated, including the target file path
