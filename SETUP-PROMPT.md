...
- selected provider and whether its local subscription CLI is authenticated;
...
PHASE 3: CHOOSE THE RENAME LEVEL

Ask whether the owner wants:

A. Persona rename only: the agent introduces itself with the chosen identity,
   while the `henry` command, HENRY_ environment prefix, framework class names,
...
PHASE 4: IMPLEMENT PRIVATE PERSONA AND CONFIGURATION
...
5. Create .env from .env.example if absent. Set only configuration keys confirmed
